...
internal class NetworkStateImp : NetworkState {
    ...
    override var isConnected: Boolean = false
        set(value) {
            field = value
            NetworkEventManager.notify(if (value) NetworkEvent.ConnectivityAvailable else NetworkEvent.ConnectivityLost)
        }
    override var linkProperties: LinkProperties? = null
        set(value) {
            val event = NetworkEvent.LinkPropertyChanged(field)
            field = value
            NetworkEventManager.notify(event)
        }
...
object NetworkEventManager : LiveData<NetworkEvent>() {
